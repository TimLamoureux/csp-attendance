 ...
 */
class Ca_Admin_ActDeact extends Ca_Admin_Base {
	...
	public function initialize() {
		...
		register_activation_hook( CA_TEXTDOMAIN . '/' . CA_TEXTDOMAIN . '.php', array( __CLASS__, 'activate' ) );
		register_deactivation_hook( CA_TEXTDOMAIN . '/' . CA_TEXTDOMAIN . '.php', array( __CLASS__, 'deactivate' ) );
	...
	public static function upgrade_procedure() {
		if ( is_admin() ) {
			$version = get_option( 'csp-attendance-version' );
			if ( version_compare( CA_VERSION, $version, '>' ) ) {
				update_option( 'csp-attendance-version', CA_VERSION );
				delete_option( CA_TEXTDOMAIN . '_fake-meta' );
			}
		}
	}
